...
public:
	...
	__host__ __device__ PointMatrix()
	{
		matrix[0] = 1;
		matrix[1] = 0;
		matrix[2] = 0;
	...
	__host__ __device__ PointMatrix(double rotation)
	{
	...
	__host__ __device__ PointMatrix(const Point p)
	{
		matrix[0] = p.X;
		matrix[1] = p.Y;
	...
	__host__ __device__ Point apply(const Point p) const
	{
		return Point(p.X * matrix[0] + p.Y * matrix[1], p.X * matrix[2] + p.Y * matrix[3]);
	}

	__host__ __device__ Point unapply(const Point& p) const
	{
...
public:
	...
	int rotation;
	int layerNr;
	__host__ __device__ pointPtr(int a, int b, int c) :location(a), rotation(b), layerNr(c) {}
};
#endif//INT_POINT_H
