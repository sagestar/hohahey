...
namespace cura {
	...
	void fffProcessor::sendPolygonsToGui(const char* name, int layerNr, int32_t z, Polygons& polygons)
	{
	...
	bool fffProcessor::processFile(const std::vector<std::string> &files)
	{
		...
		TimeKeeper timeKeeperG;
		writeGCode(storage);
		printf("Total time elapsed by CPU to writeGCode %5.2fs.\n", timeKeeperG.restart());
	...
	bool fffProcessor::prepareModel(SliceDataStorage& storage, const std::vector<std::string> &files)
	{
		timeKeeper.restart();
	...
	void fffProcessor::writeGCode(SliceDataStorage& storage)
	{
		...
		vector< vector<Point> > scanlineStorage;
		vector< vector<int> > ptrForPoints;
		vector<int> polygonsNum;
		long long polysCnt = 0;
		clock_t start, finish;
		double duration;
		start = clock();

		bool cudaError = cudaInfill(*this, storage, scanlineStorage, ptrForPoints, polygonsNum);
		finish = clock();
		duration = (double)(finish - start) / CLOCKS_PER_SEC;
		printf("Infill time        : %f seconds\n", duration);
...
#undef SPEED_SMOOTH
			...
			for (unsigned int volumeCnt = 0; volumeCnt < storage.volumes.size(); volumeCnt++)
			{
				...
				addVolumeLayerToGCode(storage, gcodeLayer, volumeIdx, layerNr, scanlineStorage, ptrForPoints, polygonsNum, polysCnt);
		...
		printf("Copy time elapsed %5.2fs.\n", TimeKeeperIn.restart());
	...
	void fffProcessor::addVolumeLayerToGCode(SliceDataStorage& storage, GCodePlanner& gcodeLayer, int volumeIdx, int layerNr, vector< vector<Point> >& scanlineStorage, vector< vector<int> >& ptrForPoints, vector<int>& polygonsNum, long long& polysCnt)
	{
		...
		for (unsigned int partCounter = 0; partCounter < partOrderOptimizer.polyOrder.size(); partCounter++)
		{
			...
			vector<Point> modelSizeM;
			modelSizeM.push_back(Point(storage.modelMax.x, storage.modelMax.y));
			modelSizeM.push_back(Point(storage.modelMin.x, storage.modelMin.y));

			if (!config.perimeterBeforeInfill)
			{
				addInfillToGCode(part, gcodeLayer, layerNr, extrusionWidth, fillAngle, scanlineStorage, ptrForPoints, polygonsNum, polysCnt + partOrderOptimizer.polyOrder[partCounter], modelSizeM);
		...
		polysCnt += partOrderOptimizer.polyOrder.size();
		gcodeLayer.setCombBoundary(nullptr);
	}

	void fffProcessor::addInfillToGCode(SliceLayerPart* part, GCodePlanner& gcodeLayer, int layerNr, int extrusionWidth, int fillAngle, vector< vector<Point> >& scanlineStorage, vector< vector<int> >& ptrForPoints, vector<int>& polygonsNum, long long polysCnt, vector<Point> modelSizeM)
	{
		...
		if (config.sparseInfillLineDistance > 0)
		{
			...
			case INFILL_GRID:
				generateGridInfill_GPU(infillPolygons, config.sparseInfillLineDistance,
					fillAngle, scanlineStorage, ptrForPoints, polygonsNum, polysCnt, layerNr, modelSizeM);
				break;

			case INFILL_LINES:
				generateLineInfill_GPU(infillPolygons, config.sparseInfillLineDistance,
					fillAngle, scanlineStorage, ptrForPoints, polygonsNum, polysCnt,layerNr, modelSizeM);
				break;

			case INFILL_CONCENTRIC:
				generateConcentricInfill_GPU(
					scanlineStorage, ptrForPoints, part->sparseOutline, infillPolygons,
					config.sparseInfillLineDistance, polysCnt, layerNr);
				break;
			}
		}
	...
	void fffProcessor::addInsetToGCode(SliceLayerPart* part, GCodePlanner& gcodeLayer, int layerNr)
	{
	...
	void fffProcessor::addSupportToGCode(SliceDataStorage& storage, GCodePlanner& gcodeLayer, int layerNr)
	{
	...
	void fffProcessor::addWipeTower(SliceDataStorage& storage, GCodePlanner& gcodeLayer, int layerNr, int prevExtruder)
	{
