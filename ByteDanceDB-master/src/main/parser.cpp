...
//输入SQL语句，进行语法解析，生成的result_存放语法树，并对其中进行合法性检验
...
//定义基类SQLStatement，析构函数为虚函数
//仅支持select、insert、update、delete、create、drop操作
bool Parser::checkMeta(const SQLStatement* stmt) {
  switch (stmt->type()) {
    case kStmtSelect:
      //父类SQLStatement转派生类SelectStatement
...
//select xxx from xxx where xxx order xxx limit xxx
bool Parser::checkSelectStmt(const SelectStatement* stmt) {
  ...
  //不支持groupBy、Union、set、with、lock等操作
